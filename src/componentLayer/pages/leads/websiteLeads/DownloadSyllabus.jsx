...
const DownloadSyllabus = () => {
  ...
  const { websiteLeadsState, DispatchwebsiteLeads, websiteLeadsState: { DownloadSyllabus } } = WebsiteLeadProvider();
  const { debouncesetSearch, debouncesetPage } = Usedebounce(DispatchwebsiteLeads);

  const handleSearch = (e) => {
    debouncesetSearch({
      context: "DOWNLOAD_SYLLABUS_LEADS",
      data: e.target.value,
    });
  };
  ...
  let currentPage = DownloadSyllabus?.page;
  const totalPages = DownloadSyllabus?.totalPages;


  const changePage = (page) => {
    debouncesetPage({ context: "DOWNLOAD_SYLLABUS_LEADS", data: page });
    currentPage = page;
    // setCurrentPage(page);
  ...
  return (
    <div>
      <BackButton heading="Download Syllabus" content="Back" />
              ...
              <div className="card-body bg_white">
                  ...
                  <div className="table-container table-scroll">
                    <table className="bg_white table table-centered align-middle table-nowrap equal-cell-table table-hover">
                      ...
                      <tbody className="bg_white">
                        ...
                        {
                          DownloadSyllabus?.paginatedDownloadSyllabus && DownloadSyllabus?.paginatedDownloadSyllabus.length > 0 ?
                            DownloadSyllabus?.loading ? "loading" : DownloadSyllabus?.paginatedDownloadSyllabus.map((item, index) => {
                ...
                <div className="mt-4 align-items-center d-flex justify-content-between row text-center text-sm-start">
                  <div className="col-sm">
                    {DownloadSyllabus.paginatedDownloadSyllabus &&
                      DownloadSyllabus.paginatedDownloadSyllabus.length >
                      0 ? (
                      DownloadSyllabus?.loading ? (
                        ...
                        <div className="text_mute pagination-text">
                          ...
                          <span className="fw-semibold">
                            {DownloadSyllabus.startDownloadSyllabus}
                          </span>
                          {"  "}
                          to{"  "}
                          <span className="fw-semibold">
                            {DownloadSyllabus.endDownloadSyllabus}
                          </span>
                          {"  "}
                          of{"  "}
                          <span className="fw-semibold">
                            {"  "}
                            {DownloadSyllabus.searchResultDownloadSyllabus}
                          </span>{" "}
                          Results
                        </div>
                      )
                      ...
                      <li className="page-item p-1">
                        <button
                          ...
                          disabled={
                            DownloadSyllabus.loading
                              ? true
                              : false || DownloadSyllabus.page === 1
                          }
                          style={{
                            cursor:
                              DownloadSyllabus.loading ||
                                DownloadSyllabus.page === 1
                                ? "not-allowed"
                                : "auto",
                          }}
                          className={`border border-1 ${DownloadSyllabus.loading
                            ? "disabled"
                            : DownloadSyllabus.page === 1
                              ? "disabled"
                              : "cursor-auto"
                        ...
                        return (
                          <li className={`page-item p-1`}>
                            <button
                              ...
                              disabled={
                                DownloadSyllabus?.loading ? true : false
                              }
                        ...
                        <button
                          ...
                          disabled={
                            DownloadSyllabus.loading
                              ? true
                              : false ||
                              DownloadSyllabus.page ===
                              DownloadSyllabus.totalPages
                          }
                          style={{
                            cursor:
                              DownloadSyllabus.loading ||
